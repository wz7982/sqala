...
object Expr:
    ...
    extension [T: Number : AsSqlExpr](expr: Expr[T])
        @targetName("plus")
        def +[R: Number : AsSqlExpr](value: R): Expr[Wrap[T, Option]] = Binary(expr, Plus, Literal(value))

        @targetName("plus")
        def +[R: Number](that: Expr[R]): Expr[Option[BigDecimal]] = Binary(expr, Plus, that)

        @targetName("minus")
        def -[R: Number : AsSqlExpr](value: R): Expr[Wrap[T, Option]] = Binary(expr, Minus, Literal(value))

        @targetName("minus")
        def -[R: Number](that: Expr[R]): Expr[Option[BigDecimal]] = Binary(expr, Minus, that)

        @targetName("times")
        def *[R: Number : AsSqlExpr](value: R): Expr[Wrap[T, Option]] = Binary(expr, Times, Literal(value))

        @targetName("times")
        def *[R: Number](that: Expr[R]): Expr[Option[BigDecimal]] = Binary(expr, Times, that)

        @targetName("div")
        def /[R: Number : AsSqlExpr](value: R): Expr[Wrap[T, Option]] = Binary(expr, Div, Literal(value))

        @targetName("div")
        def /[R: Number](that: Expr[R]): Expr[Option[BigDecimal]] = Binary(expr, Div, that)

        @targetName("mod")
        def %[R: Number : AsSqlExpr](value: R): Expr[Wrap[T, Option]] = Binary(expr, Mod, Literal(value))
