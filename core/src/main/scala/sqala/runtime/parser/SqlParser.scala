...
class SqlParser extends StandardTokenParsers:
    ...
    def select: Parser[SqlExpr.SubQuery] =
        "SELECT" ~> opt("DISTINCT") ~ selectItems ~ opt(from) ~ opt(where) ~ opt(groupBy) ~ opt(orderBy) ~ opt(limit) ^^ {
            case distinct ~ s ~ f ~ w ~ g ~ o ~ l =>
                val param = if distinct.isDefined then Some("DISTINCT") else None
                SqlExpr.SubQuery(
    ...
    def simpleTable: Parser[SqlTable] =
        ...
        opt("LATERAL") ~ ("(" ~> select <~ ")") ~ (opt("AS") ~> ident) ^^ {
            case lateral ~ s ~ alias => SqlTable.SubQueryTable(s.query, lateral.isDefined, alias)
        }
