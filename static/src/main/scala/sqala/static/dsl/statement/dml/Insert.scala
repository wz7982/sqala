...
class Insert[T, S <: InsertState](
    ...
    inline def apply[I: AsExpr as a](f: Table[T] => I): Insert[a.R, InsertTable] =
        ...
        val sqlTable: SqlTable.Standard = SqlTable.Standard(tableName, None, None, None)
...
object Insert:
    inline def apply[T <: Product]: Insert[T, InsertTable] =
        val tableName = TableMacro.tableName[T]
        val tree: SqlStatement.Insert = 
            SqlStatement.Insert(SqlTable.Standard(tableName, None, None, None), Nil, Nil, None)
        new Insert(tree)
    ...
    inline def insertByEntities[T <: Product](entities: Seq[T])(using 
        ...
        new Insert(
            SqlStatement.Insert(
                SqlTable.Standard(tableName, None, None, None), columns, values.toList, None
            )
        )