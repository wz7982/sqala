...
given valueAndQueryEqual[L, R](using CanEqual[L, R], QueryContext): CanEqual[L, Query[R]] =
    CanEqual.derived
...
extension [X](x: X)
    ...
    def in[Y](query: Query[Y])(using QueryContext, CanEqual[X, Y]): Boolean =
        compileTimeOnly
...
extension [X: Number](x: X)
    @targetName("plus")
    def +[Y: Number](y: Y)(using QueryContext): NumericOperationResult[X, Y] = compileTimeOnly
