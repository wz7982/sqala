...
object AsExpr:
    given exprAsExpr[T: AsSqlExpr, K <: ExprKind]: AsExpr[Expr[T, K]] with
    ...
    given tupleAsExpr[X: AsSqlExpr, K <: ExprKind, T <: Tuple](using
        ...
        def asExprs(x: Expr[X, K] *: T): List[Expr[?, ?]] =
            h.asExprs(x.head) ++ t.asExprs(x.tail)

    given tuple1AsExpr[X: AsSqlExpr, K <: ExprKind](using
        h: AsExpr[Expr[X, K]]
    ): AsExpr[Expr[X, K] *: EmptyTuple] with
        def asExprs(x: Expr[X, K] *: EmptyTuple): List[Expr[?, ?]] =
            h.asExprs(x.head)
...
object AsInExpr:
    given exprAsExpr[T, K <: ExprKind]: AsInExpr[Expr[T, K]] with
        def asExprs(x: Expr[T, K]): List[Expr[?, ?]] =
            x :: Nil
    ...
    given tupleAsExpr[X, K <: ExprKind, T <: Tuple](using
        h: AsInExpr[Expr[X, K]],
        t: AsInExpr[T]
    ): AsInExpr[Expr[X, K] *: T] with
        def asExprs(x: Expr[X, K] *: T): List[Expr[?, ?]] =
            h.asExprs(x.head) ++ t.asExprs(x.tail)

    given tuple1AsExpr[X, K <: ExprKind](using
        h: AsInExpr[Expr[X, K]]
    ): AsInExpr[Expr[X, K] *: EmptyTuple] with
        def asExprs(x: Expr[X, K] *: EmptyTuple): List[Expr[?, ?]] =
            h.asExprs(x.head)